...
Ball.prototype.addCollided = function(collided){
  this.collided.push(collided);
}
...
Ball.prototype.ax = function(){
  return (this.impulseX.length > 0)? this.impulseX.reduce((acc,val) => acc + val)/this.impulseX.length : 0;
}
Ball.prototype.ay = function(){
  return (this.impulseY.length > 0)? this.impulseY.reduce((acc,val) => acc + val)/this.impulseY.length : 0;
}
...
export default {
  ...
  data(){
    return{
      intervalId:null,
      balls:[
        new Ball(-40,0,80,0,"cue"),
        //new Ball(40,0,0,0,"red"),
  ...
  methods:{
    physicsSimulate(){
      ...
      this.intervalId = setInterval(function(){
        if(this.balls.filter( x=> !x.stationary()).length == 0){ clearInterval(this.intervalId); this.intervalId = null; console.log(frameCount)}
        for(let ball of this.balls.filter(x=>!x.stationary())){
          ...
          for(let otherBall of this.balls.filter(x => !ball.collided.includes(x))){
              let distance = Math.sqrt(Math.pow(otherBall.x - ball.x,2) + Math.pow(otherBall.y - ball.y,2));
              if(distance < 2*this.ballRadius){ 
                  ...
                  ball.impulseX.push((Vx -u1x*this.restitutionCoeff*Math.cos(impactAngle - initAngle) + u1y*this.restitutionCoeff*Math.sin(impactAngle - initAngle)) - ball.vx);
                  otherBall.impulseX.push((Vx -u2x*this.restitutionCoeff*Math.cos(impactAngle - initAngle) + u2y*this.restitutionCoeff*Math.sin(impactAngle - initAngle)) - otherBall.vx);
                  ball.impulseY.push((Vy - u1x*this.restitutionCoeff*Math.sin(impactAngle - initAngle) - u1y*this.restitutionCoeff*Math.cos(impactAngle - initAngle)) - ball.vy);
                  otherBall.impulseY.push((Vy - u2x*this.restitutionCoeff*Math.sin(impactAngle - initAngle) - u2y*this.restitutionCoeff*Math.cos(impactAngle - initAngle)) - otherBall.vy);
                  ball.addCollided(otherBall);
                  otherBall.addCollided(ball);
              }
          }
        }
        for(let ball of this.balls){
          ...
          ball.vx -= ((this.frictionCoeff)*ball.vx + this.staticFriction)*Math.cos(angle)*(1/60) - aX;
          ball.vy -= ((this.frictionCoeff)*ball.vy + this.staticFriction)*Math.sin(angle)*(1/60) - aY;  
